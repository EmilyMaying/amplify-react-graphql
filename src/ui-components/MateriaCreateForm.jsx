...
export default function MateriaCreateForm(props) {
  ...
  const [fornecedor, setFornecedor] = React.useState(initialValues.fornecedor);
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    setNome(initialValues.nome);
    setCusto(initialValues.custo);
    setFornecedor(initialValues.fornecedor);
    setErrors({});
  };
  const validations = {
    nome: [{ type: "Required" }],
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.fornecedor ?? value;
          }
          if (errors.fornecedor?.hasError) {
            runValidationTasks("fornecedor", value);
          }
          setFornecedor(value);
        }}
        onBlur={() => runValidationTasks("fornecedor", fornecedor)}
        errorMessage={errors.fornecedor?.errorMessage}
        hasError={errors.fornecedor?.hasError}
        {...getOverrideProps(overrides, "fornecedor")}
      ></TextField>
      <Flex
